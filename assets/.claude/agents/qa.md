...
2. **Update stage**: change the PRD's stage to `testing-round1` and run build_status
3. **Create the TEST-PLAN**: `docs/test-plan/TEST-PLAN-XXX.md` (XXX matches the PRD number)
4. **Design test cases** (generated from 4 sources):
...
6. **Write E2E tests**: refer to the playwright-testing skill
...
8. **Update the "Round-1 Results" section of the TEST-PLAN**: N passed, M failed, the specific failing cases
9. **If there are failures**:
...
10. **If everything passes**: change the stage to `awaiting-deploy-approval` and notify pm
